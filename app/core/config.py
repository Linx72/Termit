...
class Settings:
    ...
    agent_verify_timeout_seconds: int = 120
...
def get_settings() -> Settings:
    return Settings(
        ...
        agent_verify_timeout_seconds=max(
            1,
            min(300, int(os.getenv("TERMIT_AGENT_VERIFY_TIMEOUT_SECONDS", "120"))),
        ),
        ...
        in {"1", "true", "yes", "on"},
        ollama_warm_on_startup=os.getenv("TERMIT_OLLAMA_WARM_ON_STARTUP", "true").lower()
        in {"1", "true", "yes", "on"},
        ...
        tool_loop_metrics_recent_run_window=max(
