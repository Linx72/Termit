...
def required_role(method: str, path: str) -> str:
    ...
    if path.startswith("/api/ops/beta-metrics") and method == "GET":
        return "viewer"
    if path.startswith("/api/ops/plan-status") and method == "GET":
        return "viewer"
    if path.startswith("/api/ops/reload-dev-metrics-seed") and method == "POST":
        return "operator"
    ...
    if path.startswith("/api/tools/execute") and method == "POST":
        return "operator"
    if path.startswith("/api/tools/apply_patch") and method == "POST":
        return "operator"
