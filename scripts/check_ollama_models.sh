...
collect_from_file() {
  local file="$1"
  [[ -f "$file" ]] || return 0
  grep -E '^(TERMIT_(DEFAULT|CODE|ANALYSIS|DEFAULT_FALLBACK|CODE_FALLBACK|ANALYSIS_FALLBACK|FAST|FRONTIER_FALLBACK)_MODEL|TERMIT_RETRIEVAL_EMBED_MODEL|TERMIT_FINETUNE_OUTPUT_MODEL)=' "$file" \
    | cut -d= -f2- \
    | tr -d '\r"' \
    | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'
}
